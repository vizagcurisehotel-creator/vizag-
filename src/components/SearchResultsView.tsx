    ...
    <div style={{ background: 'var(--luxury-black)', minHeight: '100vh', paddingBottom: '120px' }}>
      ...
      <div className="container" style={{ marginTop: '80px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '60px' }}>
          {rooms.map((room) => (
            <div key={room.id} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '50px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <h3 style={{ fontSize: '2rem', color: 'var(--luxury-white)', fontFamily: 'var(--font-heading)', marginBottom: '25px' }}>{room.name}</h3>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--luxury-pearl)', opacity: 0.6, fontSize: '0.85rem' }}>
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                  <div>
                    {room.original_price && room.original_price > room.price && (
                      <div style={{ color: 'var(--luxury-pearl)', opacity: 0.4, fontSize: '1rem', textDecoration: 'line-through', marginBottom: '5px' }}>
                    ...
                    <div style={{ fontSize: '0.6rem', color: 'var(--luxury-pearl)', opacity: 0.5, textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '5px' }}>Per Enchanted Night</div>
                  </div>
                  <button 
                    onClick={() => onBook(room)}
                    className="gold-btn" 
